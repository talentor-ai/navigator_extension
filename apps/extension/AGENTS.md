...
- `@vitejs/plugin-react` is installed even though transforms use `@vitejs/plugin-react-swc`: CRXJS hard-imports `@vitejs/plugin-react` to fill `contentScripts.preambleCode`. Without it that import throws, CRXJS silently falls back to `preambleCode: false`, and the content script fails in dev with `@vitejs/plugin-react-swc can't detect preamble`, so the launcher never mounts. Keep the dependency, and restart `bun run dev:extension` after changing it (the preamble is resolved at server start).
